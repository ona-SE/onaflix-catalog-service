...
router.get('/:id/recommendations', async (req, res) => {
  ...
  try {
    ...
    const params = new URLSearchParams({ genre: movie.genre, excludeId: String(movie.id) });
    const resp = await fetch(`${recommendationUrl}/api/recommend?${params}`, { redirect: 'error' });
    if (!resp.ok) throw new Error(`Recommendation service returned ${resp.status}`);
